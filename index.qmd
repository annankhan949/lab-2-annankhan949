...
using Random
test_station = rand(eachrow(stations))
test_stnid = test_station.stnid
random_rainfall = @chain rainfall_data begin
	@filter(stnid == !!test_stnid)  # <1>
end
random_rainfall
```
...
plot_time_series(test_station, random_rainfall)
...
μ = 2   # Location parameter - shifts the distribution left/right
σ = 0.3    # Scale parameter - controls spread (must be > 0)
ξ = 0  # Shape parameter - controls tail behavior
...
## A station with an apparent positive trend could be station 41-8647, this is with a grain of salt however as there is less data for this station but it looks to be increasing over time
...
## A potential negative trend can be spotted with station 41-3060 as it has a ever so slight downturn.
...
- Final: $\mu = 2.0\ldots$ $\sigma =0.3 \ldots$, $\xi = 0\ldots$

**Parameter Effects:**

**What happened when you increased/decreased $\mu$ (location parameter)?**

## Increasing and decreasing shifted the plot from left and right. Decreasing it to 2 caused the return period plot to lower its right side significantly  
...
# decreasing the scale parameter concentrated more values in one area and shortened the return period overall



**What happened when you changed $\xi$ (shape parameter)?** 

# 0 looked like it brought the right end of the tail lower and failed to account for some extreme data in the return period 
...
# I would say it does not fit well, it looks to be underestimating the observation consistently and significantly underestimating extremes.
